...
export const allQuests: Quest[] = [
  // Starting Quests (Dam Battlegrounds Unlocked)
  {
    id: 'db-1',
  ...
  // Multiple quest branches from here
  {
    id: 'ml-2',
    name: 'A Bad Feeling',
    location: 'Dam Battlegrounds, Spaceport, Buried City',
  ...
  // Continuing with more branches from the quest tree...
  {
    id: 'bc-6',
  ...
  // Continuing with more quest tree branches...
  {
    id: 'sp-3',
  ...
  // Blue Gate Introduction (Unlocks at 18 rounds)
  {
    id: 'bg-1',
  ...
  // Stella Montis Introduction (Unlocks at 24 rounds)
  {
    id: 'sm-2',
...
// Legacy location-based grouping for backwards compatibility
...
// Populate questsByLocation from allQuests
allQuests.forEach(quest => {
  const loc = quest.location.toLowerCase();
  if (loc.includes('dam')) {
    questsByLocation['Dam Battlegrounds'].push(quest);
  } else if (loc.includes('spaceport')) {
    questsByLocation['Spaceport'].push(quest);
  } else if (loc.includes('buried city')) {
    questsByLocation['Buried City'].push(quest);
  } else if (loc.includes('blue gate')) {
    questsByLocation['The Blue Gate'].push(quest);
  } else if (loc.includes('stella montis')) {
    questsByLocation['Stella Montis'].push(quest);
  } else {
    questsByLocation['Multiple Locations'].push(quest);
  }
});
