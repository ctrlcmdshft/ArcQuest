...
export default function Home() {
  ...
  // Load completed quests from URL or localStorage
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const sharedQuests = params.get('completed');
  ...
  const copyShareLink = () => {
    const questIds = Array.from(completedQuests).join(',');
    const url = `${window.location.origin}${window.location.pathname}?completed=${questIds}`;
  ...
  const filteredQuests = allQuests.filter(quest => {
    ...
    const matchesCompletedFilter = !hideCompleted || !completedQuests.has(quest.id);
    
    return matchesLocation && matchesCategory && matchesSearch && matchesCompletedFilter;
  });
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="mt-4">
            <div className="flex justify-between items-center text-sm mb-2">
              ...
              <div className="flex items-center gap-4">
                <button
                  ...
                  className="px-3 py-1 bg-orange-500/20 hover:bg-orange-500/30 border border-orange-500/50 rounded-lg text-orange-400 text-xs font-medium transition-colors flex items-center gap-2"
                >
            ...
            {showCopyNotification && (
              <div className="text-xs text-green-400 text-right mb-1">✓ Link copied to clipboard!</div>
            )}
          ...
          <div className="flex items-center justify-between mb-4">
            <label className="flex items-center space-x-2 cursor-pointer">
              <input
                ...
                className="w-4 h-4 text-orange-500 bg-gray-900 border-gray-600 rounded focus:ring-orange-500 focus:ring-2"
              />
              <span className="text-sm font-medium text-gray-300">Hide completed quests</span>
            </label>
          </div>
      ...
      <footer className="bg-black/40 border-t border-gray-700 py-6 mt-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-gray-400 text-sm">
          <p>ARC Raiders Quest Tracker v1.5.0 • {totalQuests} Total Quests</p>
          ...
          <p className="mt-1 text-gray-500 text-xs">Last updated: November 30, 2025</p>
        </div>
      </footer>
    </div>
  );
